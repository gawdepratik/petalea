...
(function initPincodeLookup() {
  ...
  const areaSelect = document.getElementById("checkoutArea");
  ...
  pincodeInput.addEventListener("input", async () => {
    ...
    if (result && result.city) {
      ...
      if (areaSelect && result.areas && result.areas.length) {
        areaSelect.innerHTML = result.areas.map((a) => `<option value="${a}">${a}</option>`).join("");
      } else if (areaSelect) {
        areaSelect.innerHTML = `<option value="">No specific areas found</option>`;
      }
    }
  });
})();
